class Service::CodePortingCSharp2Java < Service
  ...
  password :codeporting_password, :github_access_token
  ...
  def check_configuration_options(data)
    ...
    raise_config_error 'GitHub Access Token must be provided for committing changes back to GitHub' if data['github_access_token'].blank?
  end
end
