class Service::TargetProcessOnDemand < Service
  string    :base_url, :username, :project_id
  password  :password

  maintained_by :github => 'nzben'
  url "https://www.targetprocess.com"
  logo_url "https://www.targetprocess.com/img/common/logo-head-new.png"
...
private
  ...
  def execute_command(author, entity_id, command, commit_message)
    ...
    # get the user's id
    res = http_get "api/v1/Users", {:where => "(Email eq '%s')" % author, :format => 'json'}
    valid_response?(res)
    author_id = begin JSON.parse(res.body)['Items'][0]['Id'] rescue nil end
